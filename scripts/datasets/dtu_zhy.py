...
# for invdepth, T are divded by 100.0
...
class MVSDataset(Dataset):
    ...
    def read_cam_file(self, filename):
        ...
        extrinsics[:3, 3] /= 100.0
        # intrinsics: line [7-10), 3x3 matrix
        intrinsics = np.fromstring(' '.join(lines[7:10]), dtype=np.float32, sep=' ').reshape((3, 3))
        # depth_min & depth_interval: line 11
        depth_min = float(lines[11].split()[0]) / 100.0
        depth_interval = float(lines[11].split()[1]) * self.interval_scale / 100.0
    ...
    def read_depth_hr(self, filename):
        # read pfm depth file
        #w1600-h1200-> 800-600 ; crop -> 640, 512; downsample 1/4 -> 160, 128
        depth_hr = np.array(read_pfm(filename)[0], dtype=np.float32) / 100.0
    ...
    #     target_h, target_w = 512, 640
    #     start_h, start_w = (h - target_h)//2, (w - target_w)//2
    #     depth = depth[start_h: start_h + target_h, start_w: start_w + target_w]
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                ...
                # mask = self.read_mask(mask_filename_hr)
                # depth = self.read_depth(depth_filename_hr)
